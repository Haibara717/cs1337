// This program demonstrates a simple class.
#include <iostream>
using namespace std;

#define PI 3.14
// Circle class declaration.
class Circle
{
   private:
      double radius;

   public:
      void setRadius(double r)// setRadius assigns a value to the radius member.
      {
          if(r>0)
          {
              radius = r;
          }
          else
          {
              cout<<"Radius must be a positive value";
              exit(0);
          }
      }
      
      double getRadius() const// getRadius assigns a value to the radius member.
      {
          return radius;
      }
      
      double getDiameter() const// getDiameter 
      {
          return radius * 2;
      }
      
      double getArea() const// getArea
      {
          return radius * radius * PI;
      }
};




int main()
{
   Circle c1;     // Define an instance of the Circle class
   double rad, rad1;  // Local variable for radius

   // Get the radius from the user.
   cout << "This program will calculate the area of a circle.\n";
   cout << "What is the radius? ";
   cin >> rad;
   
   cout << "What is the radius of the object pointed by c2? ";
   cin >> rad1;


   // Store the radius of the circle in the c1 object.
   c1.setRadius(rad);

   // Display the circle's data.
   cout << "\nHere is the circle's data:\n";
   cout << "Radius: " <<c1.getRadius()  << endl;
   cout << "Diameter: " <<c1.getDiameter() << endl;
   cout << "Area: " <<c1.getArea()  << endl;

    Circle *c2 = new Circle;//dont forget to delete it 

   // Store the radius of the circle in the c2 object.
   c2->setRadius(rad1);//c2 is a pointer so we need to dereference it

   // Display the circle's data.
   cout << "\nHere is the circle's data through the pointer:\n";
   cout << "Radius: " <<c2->getRadius()  << endl;
   cout << "Diameter: " <<c2->getDiameter()  << endl;
   cout << "Area: " <<c2->getArea() << endl;
   
   delete c2;// Free dynamically allocated memory

   return 0;
}
